...
/**
 * Send the poll owner a summary email with results when a poll ends.
 *
 * If the Resend client or the configured sender address is not available, the function exits without sending.
 * The email includes a plain-text and rich HTML summary and highlights the top three options by votes.
 *
 * @param param0.poll - The poll object that has ended
 * @param param0.owner_email - Recipient email address for the poll owner
 */
...
/**
 * Builds a plain-text summary of a poll's results for inclusion in an email.
 *
 * The generated text includes a header, total votes, poll type, optional
 * average rating or text response count (when applicable), an optional
 * "Top options" list, and a link to view full results.
 *
 * @returns The plain-text email body summarizing the poll results
 */
...
/**
 * Generates a complete HTML email body summarizing a finished poll, including statistics, top options, and a call-to-action linking to full results.
 *
 * @param poll - The poll object containing title, type, totals, ratings, and option data used in the summary
 * @param result_url - URL linking to the poll's full results
 * @param top_options - Array of the top poll options (ordered by votes) to display in the email
 * @returns An HTML string suitable for use as the email body
 */
...
/**
 * Escapes special characters in a string so it can be safely embedded in HTML.
 *
...
/**
 * Build a public URL for an S3 object key using the configured public S3 base URL.
 *
 * Removes any trailing slashes from the configured base URL and any leading slashes from `key`, then concatenates them with a single slash.
 *
 * @param key - The S3 object key or path (may include leading slashes)
 * @returns The fully qualified public URL for the given S3 key
 */
