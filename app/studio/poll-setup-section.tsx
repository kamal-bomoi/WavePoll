...
/**
 * Renders the poll setup UI used to configure title, description, type-specific options, scheduling, and optional owner/status fields.
 *
 * @param form - The StudioForm instance managing poll fields and validation.
 * @param on_type_change - Callback invoked with the new poll type when the type selector changes.
 * @param image_files - Array of uploaded image files (or null) corresponding to image options.
 * @param on_change_image_file - Callback invoked with (index, file) when an image option file is changed.
 * @param on_add_image_option - Callback invoked to add a new image option.
 * @param on_remove_image_option - Callback invoked with (index) to remove an image option.
 * @param image_option_keys - Optional array of image keys used for previews (defaults to []).
 * @param show_owner_email - Whether to render the "Notification email" field (defaults to true).
 * @param show_status - Whether to render the poll status select (defaults to true).
 * @returns The JSX element containing the full poll setup form (title, description, type selector, options UI, per-type controls, and end time picker).
 */
