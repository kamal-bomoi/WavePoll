...
/**
 * Render an editable poll editor UI and handle updating a poll draft.
 *
 * Renders a form prefilled from `poll`, allows changing title, description, type,
 * options (including image options and files), reactions, owner email, status,
 * and end date. Guards editing to the poll owner, to draft polls, and to polls
 * that haven't ended. Submits user-confirmed updates via a mutation, updates
 * cached queries on success, shows a success toast, and navigates back to the
 * studio list.
 *
 * @param poll - The poll to edit.
 * @param is_owner_view - Whether the current user is the poll owner (controls edit access).
 * @param initial_owner_email - The initial owner email value or `null` when none.
 * @returns The edit-poll form UI as a React element.
 */
