...
/**
 * Render the voting room page for the poll identified by the route parameter `poll_id`.
 *
 * Displays a loading indicator while the poll query is pending, an error alert if the query fails,
 * or the poll UI when data is available. The UI shows the poll status (and a realtime indicator when
 * the poll is live and not ended), current viewer count, poll title and description, the voting form
 * (with owner view when applicable), a last-updated relative timestamp, and a button to create a new poll.
 *
 * @returns The React element for the voting room page for the specified poll.
 */
