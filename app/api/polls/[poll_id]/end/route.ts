...
/**
 * Handle a poll-end webhook: ensure a single emission of the poll-ended event and optionally queue an owner notification.
 *
 * Checks the poll's end state, creates an idempotency lock in Redis to prevent duplicate emissions, emits the poll-ended event, and, if configured, queues a poll-ended summary email for the poll owner.
 *
 * @param req - The incoming Request object for the webhook (used for request metadata in error reporting).
 * @param context - Route context whose `params` promise resolves to an object containing `poll_id`.
 * @returns A Response with JSON `{ ok: true }` when processing completes.
 * @throws Re-throws any error after reporting it to Sentry.
